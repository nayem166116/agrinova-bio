...
- Add brand-assets/logo.png and brand-assets/icon.png before launch.

## IMPORTANT: how to deploy

The files in THIS folder (index.html, vercel.json, assets/) must sit at the ROOT of the deployment.

If you get a `404: NOT_FOUND` on Vercel, it is because the project root contains an `agrinova-bio` folder instead of `index.html` directly.

### Vercel drag-and-drop

Drag the `agrinova-bio` FOLDER itself onto Vercel (not a zip containing it). Vercel then uses the folder contents as the root.
